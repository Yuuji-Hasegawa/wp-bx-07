...
function get_front_news()
{
    ...
    if ($the_query->have_posts()) {
        ...
        $output .= '<div class="u-text-center js-fade-up">
          <a class="o-box o-box--button o-box--rect-button u-font-en-con"
            href="' . home_url('/news/') . '">
            MORE
          </a>
        </div>';
...
function get_front_product()
{
    ...
    $the_query = new WP_Query($args);
    if ($the_query->have_posts()) {
        $output = '<ul class="o-grid o-grid--tri u-mb-xl">';
        while ($the_query->have_posts()) {
            $the_query->the_post();
            $output .= '<li>
                <a href="' . get_the_permalink() . '" class="c-puton c-puton--filter c-pict-link">
                ' . get_thumb_sq() .
                '<div class="c-puton__inner o-cover u-text-white">
                  <div class="u-text-center u-block-bottom u-pr-m u-pl-m">
                    <span class="c-display-xs u-text-weight-b">' . get_the_title() . '</span>
                  </div>
                </div>
              </a>
            </li>';
    ...
    if ($output) {
        ...
        return '<p class="c-content-l js-fade-up">商品はまだありません。</p>';
    }
}
...
function get_related_product()
{
    ...
    $the_query = new WP_Query($args);
    if ($the_query->have_posts()) {
        $output = '<h2 class="c-sec-heading u-text-weight-b js-pull-view">関連商品</h2>
        <ul class="o-grid o-grid--tri">';
        while ($the_query->have_posts()) {
            $the_query->the_post();
            $output .= '<li>
